Perfect, this is exactly what we need to bridge your CoMix Faster R-CNN detections + VLM page JSON into a train-ready dataset.

Below is a crisp data contract plus a conversion script that:
- Parses your COCO-ish detections (images, annotations, categories)
- Runs NMS and filtering on panel/text/balloon/character boxes
- Assigns detections to panels
- Infers reading order + adjacency
- Fuses in your VLM per-panel text as a fallback (until OCR is added)
- Emits the page-level JSON in the format the CLOSURE-Lite pipeline expects

Summary of what you need to prepare
- You already have: CoMix detections with categories: panel(1), character(2), balloon(3), text(4), onomatopoeia(5), link_sbsc(6), face(7)
- Provide (for each page):
  - Page image path and size (we can read size from image if missing)
  - Detection JSON (COCO-like)
  - Optional: your VLM narrative JSON for that page (to populate panel text right now)
- Optional but recommended:
  - OCR on text/balloon regions later (PaddleOCR), to replace or augment the VLM text

DataSpec v0.3 (what our model expects per page)
- page_id: stable string (we’ll derive from file path)
- page_image_path: absolute or relative path to the page image
- page_size: width, height in pixels
- panels: list of
  - panel_id: unique
  - panel_coords: [x, y, w, h] int pixels
  - order_index: int (reading order)
  - neighbors: {left: [], right: [], above: [], below: []} (optional, we’ll also build adj_mask at load)
  - text: {dialogue: [], narration: [], sfx: []} (filled from VLM now; later from OCR + balloon)
  - ocr_tokens: [] (placeholder now; later from OCR)
  - character_coords: [[x,y,w,h], ...] (from category 2 and 7)
  - face_coords: [[x,y,w,h], ...] (from category 7)

How we’ll use your detections
- category_id 1: panel → we’ll NMS and keep the final panel list
- category_id 3: balloon → we’ll use these as regions for dialogue assignment (later when OCR is on)
- category_id 4: text → geometry for OCR (later). For now they help count/coverage, but not needed for training.
- category_id 5: onomatopoeia → sfx regions (later OCR)
- category_id 2, 7: character and face → composition features (shot scale, subject position)
- category_id 6: link_sbsc → ignored for now (used later to link balloons to speakers)

Heuristics/thresholds
- panel_score_thr: 0.75 (raise to 0.85 if many false positives)
- panel_nms_iou: 0.25
- text/balloon/char_nms_iou: 0.3–0.5
- Remove panel candidates with extremely thin bboxes (w<32 or h<32) or edge strips that look like gutters/headers

Reading order and adjacency (no adjacency given)
- We infer a robust Z-path: vertical row grouping by overlap, then left-to-right within row (or RTL switch for manga)
- Adjacency: nearest left/right/above/below with overlap constraints + “next in reading order” + kNN fallback

Converter: COCO detections + optional VLM → DataSpec v0.3
Paste this as coco_to_dataspec.py.
...
def build_pages_from_coco(coco_path: str, vlm_pages_dir: str=None,
    ...
    for img_id, anns in anns_by_img.items():
        ...
        if vlm_pages_dir:
            # Find matching VLM json by file stem
            stem = os.path.splitext(os.path.basename(path))[0]
            candidates = [f for f in os.listdir(vlm_pages_dir) if f.startswith(stem) and f.endswith('.json')]
            if candidates:
                with open(os.path.join(vlm_pages_dir, candidates[0]), 'r', encoding='utf-8') as vf:
                    vlm = json.load(vf)
...
if __name__ == '__main__':
    ...
    pages = build_pages_from_coco(args.coco, vlm_pages_dir=args.vlm_dir)
    for p in pages:
...
Where your sample fits
- The lines like {'category_id': 1, 'bbox': [...], 'score': ...} will be filtered/NMS’d into a final panel list for the page E:/.../Abe Sapien ..._00023.jpg.
- category_id=4 (text) and 5 (onomatopoeia) go into raw_text_regions for future OCR.
- category_id=2 (character) and 7 (face) enrich composition features.
- Your VLM page JSON with panels[1..6] will be mapped in reading order: we assume panel_number sequences match Z-path; if counts differ we pad/trim gracefully.

Minimal preprocessing you need before training
- Run the converter above to produce DataSpec v0.3 JSON files (one per page).
- Ensure each page JSON points to a valid page_image_path.
- Optional (recommended soon): run OCR and fill ocr_tokens + better bucket text using balloon_coords.
  - Quick start: PaddleOCR (very good on comics), then assign tokens to panels by containment; tokens overlapping balloon_coords → dialogue; tokens inside panel but outside balloons → narration; large skewed tokens overlapping onomatopoeia regions → sfx.

Why this works with CLOSURE-Lite now
- Our dataset loader already supports:
  - Cropping panels from page_image_path via panel_coords
  - Using text buckets (dialogue/narration/sfx) to build the panel’s text string
  - Computing composition features from panel size and character/face coords
  - Inferring reading order and adjacency if absent (we’re filling order_index here anyway)

Quality tips for your detections
- Turn up panel_thr to 0.85 if you see gutter/header false positives (e.g., [38,0,967,84]).
- After conversion, run a validator that draws panel boxes and order indices; quick sanity spot-check will save time.

Optional next step: adding OCR
- We can bolt on a small OCR module that:
  - Runs PaddleOCR per page
  - Assigns tokens to panels via polygon/bbox containment
  - Uses balloon_coords to bucket dialogue vs narration vs sfx
  - Populates ocr_tokens and refines text.dialogue/narration/sfx
- If you want, I’ll add a --ocr flag and ship a ready-to-run script.

Want me to tailor this to your exact files?
- If you drop the detections.json (or a small shard with your Abe Sapien page) and the single VLM page JSON to a folder, I’ll run the converter on them and return the resulting DataSpec page JSON for you to plug into the training pipeline.