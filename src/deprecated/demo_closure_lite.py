"""
Demo script for CLOSURE-Lite model
Showcase the model's comic understanding capabilities
"""
...
def demo_comic_understanding(model, dataloader, device, output_dir="demo_output"):
    ...
    print("🎨 CLOSURE-Lite Comic Understanding Demo")
    print("=" * 50)
...
def create_demo_visualization(page_data, panel_embeddings, reading_order, attention_weights, output_path):
    ...
    # Panel embeddings t-SNE (if enough panels)
    ax4 = plt.subplot(2, 3, 5)
    if len(panel_embeddings) > 3:  # Need at least 4 panels for t-SNE
        try:
            from sklearn.manifold import TSNE
            # Set perplexity to be less than number of samples
            perplexity = min(30, len(panel_embeddings) - 1)
            tsne = TSNE(n_components=2, random_state=42, perplexity=perplexity)
            embeddings_2d = tsne.fit_transform(panel_embeddings.cpu().numpy())
            
            scatter = ax4.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1], 
                                c=range(len(embeddings_2d)), cmap='Set3', s=100)
            ax4.set_title('Panel Embeddings (t-SNE)', fontsize=12, fontweight='bold')
            ...
            ax4.set_title('Panel Embeddings', fontsize=12, fontweight='bold')
    else:
        ax4.text(0.5, 0.5, f'Need 4+ panels\nfor t-SNE\n(has {len(panel_embeddings)})', ha='center', va='center', fontsize=12)
        ax4.set_title('Panel Embeddings', fontsize=12, fontweight='bold')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CLOSURE-Lite Demo')
