...
def print_class_distribution(dataset_name, json_file):
    ...
    categories = {"story": 0, "cover": 0, "credits": 0, "advertisement": 0, "art": 0, "text": 0, "back_cover": 0}
    ...
    total_pages = sum(categories.values())
    if total_pages == 0:
        print(f"\n{dataset_name} distribution: No pages found for defined categories.")
        return

    print(f"\n{dataset_name} distribution:")
    for category, count in categories.items():
        print(f"  - {category}: {count/total_pages:.2f}")
...
def main(run, gpu_id = 0,train=True, lr = 1e-4, dropout_p=0.4, epochs = 10, batch_size=32, model_id='openai/clip-vit-large-patch14', 
    ...
    # Reduced batch size from 64 to 16 to relieve GPU memory pressure during text embedding
    bb_batch_size = 16
    ...
    if window_size is not None:
        ...
        all_identifiers = results['all_identifiers']
        all_probs = results['all_probabilities']
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm.tqdm(test_loader, desc="Evaluation")):
                ...
                # Note: Probabilities are not calculated in this non-sliding-window mode
                all_probs = []
    ...
    # Explicitly provide the labels to handle cases where a class is not present in the test data
    report_labels = list(range(len(class_names)))
    print(classification_report(all_labels, all_preds, target_names=class_names, labels=report_labels, zero_division=0))
    ...
    # Explicitly provide the labels to confusion_matrix to ensure all classes are represented
    cm_labels = list(range(len(class_names)))
    cm = confusion_matrix(all_labels, all_preds, labels=cm_labels)
    cm_normalized = cm.astype(np.float32) / (cm.sum(axis=1, keepdims=True) + 1e-8) # Add epsilon for safety
    print("Confusion Matrix Normalized:")
    print(cm_normalized)
    
    per_class_acc = cm.diagonal() / (cm.sum(axis=1) + 1e-8) # Add epsilon for safety
    ...
    if len(all_probs) > 0:
        results_path = os.path.join(out_dir, f'evaluation_results_{model_name}.npz')
        ...
        try:
            ...
            df = pd.DataFrame()
            df['identifier'] = ids
            df['book_id'] = [id.split('_page_')[0] for id in ids]
            df['page_number'] = [int(id.split('_page_')[1]) for id in ids]
            df['ground_truth_label'] = [class_names[g] for g in gts]
            df['predicted_label'] = [class_names[p] for p in preds]

            prob_cols = [f'prob_{cn}' for cn in class_names]
            df[prob_cols] = np.round(probs, 3)

            csv_path = results_path.replace('.npz', '.csv')
            df.to_csv(csv_path, index=False)
            print(f"Evaluation results also saved to {csv_path}")
        except ImportError:
            print("Pandas not installed. Skipping CSV conversion. Install with 'pip install pandas'.")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    main(run,
        ...
        augmentations=config_data["config"]["augmentations"],
        num_aug_copies = config_data["config"]["num_aug_copies"], 
        num_synthetic_books = config_data["config"]["num_synth_books"], 
    )
