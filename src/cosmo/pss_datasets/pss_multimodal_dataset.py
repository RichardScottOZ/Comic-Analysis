...
class PSSMultimodalDataset(PSSDataset):
    ...
    def _extract_emb_batch(self, ocr_paths, batch_size=32):
        ...
        for i in tqdm.tqdm(range(0, len(ocr_paths), batch_size), 
                        desc="Extracting text embeddings", unit="batch"):
            ...
            try:
                ...
                for path in valid_paths:
                    ...
                    if ocr_data and 'OCRResult' in ocr_data:
                        # The content of OCRResult is a dictionary, we need to serialize it to a string
                        # for the sentence transformer model.
                        ocr_text = json.dumps(ocr_data['OCRResult'])
                        batch_text.append(ocr_text)
                    else:
                        # If JSON is invalid or doesn't have OCRResult, append empty string
                        print(f"Warning: Could not find 'OCRResult' in {path}. Using empty text.")
                        batch_text.append("")
