"""
Generate pure TEXT embeddings for comic datasets and store in Zarr format.
Based on generate_embeddings_zarr_claude.py with modality isolation for TEXT ONLY.

TEXT MODALITY ABLATION:
- Uses actual text inputs (input_ids, attention_mask) from the dataset
- Images are set to dummy zeros (blank/black images)
- Compositional features (comp_feats) are set to dummy zeros
- P_flat is generated directly from model.atom.text(input_ids, attention_mask), bypassing GatedFusion
- The resulting text-only P_flat is passed to model.han.panels_to_page for page embeddings

Usage example:
    python generate_text_embeddings_zarr_claude.py \
...
def process_batch(model, batch, device):
    """
    Process a batch and extract TEXT-ONLY embeddings.
    
    Text Modality Ablation Strategy:
    - Set images to dummy zeros (blank images)
    - Use actual input_ids and attention_mask
    - Set comp_feats to zero tensors
    - Directly use model.atom.text(input_ids, attention_mask) to get P_flat, bypassing GatedFusion
    ...
    with torch.no_grad():
        ...
        # TEXT ONLY: Get text embeddings directly, bypassing GatedFusion
        P_flat = model.atom.text(input_ids, attention_mask)
        P = P_flat.view(B, N, -1)
        
        # 2. Page-level Understanding (using text-only panel embeddings)
        E_page, attention_weights = model.han.panels_to_page(P, batch['panel_mask'])
        
        # 3. Reading Order Prediction (from text-only embeddings)
