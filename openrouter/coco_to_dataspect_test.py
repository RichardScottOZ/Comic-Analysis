...
def build_pages_from_coco(coco_path: str, vlm_pages_dir: str=None,
    ...
    print(f"Processing {total_images} images (out of {len(anns_by_img)} total in dataset)")
    
    for img_id, anns in tqdm(anns_by_img.items(), total=total_images, desc="Processing images"):
        # Check limit
        if limit is not None and processed_count >= limit:
            print(f"\nReached limit of {limit} images, stopping processing")
            break
        ...
        pages.append(page)
        processed_count += 1
        # Removed individual print since tqdm shows progress

    print(f"\nCompleted processing {len(pages)} pages successfully")
    return pages
