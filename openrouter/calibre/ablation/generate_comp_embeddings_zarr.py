"""
Generate pure COMPOSITIONAL embeddings for comic datasets and store in Zarr format.
Based on generate_embeddings_zarr_claude.py with modality isolation for COMPOSITIONAL ONLY.

COMPOSITIONAL MODALITY ABLATION:
- Uses actual compositional features (panel coordinates, layout) from the dataset
- Images are set to dummy zeros (blank/black images)
- Text inputs (input_ids, attention_mask) are set to dummy/neutral values (zeros)
- P_flat is generated directly from model.atom.comp(comp_feats), bypassing GatedFusion
- The resulting comp-only P_flat is passed to model.han.panels_to_page for page embeddings

Usage example:
    python generate_comp_embeddings_zarr_claude.py \
...
def process_batch(model, batch, device):
    """
    Process a batch and extract COMPOSITIONAL-ONLY embeddings.
    
    Compositional Modality Ablation Strategy:
    - Set images to dummy zeros (blank images)
    - Set input_ids and attention_mask to dummy neutral values (zeros)
    - Use actual comp_feats (panel coordinates and layout)
    - Directly use model.atom.comp(comp_feats) to get P_flat, bypassing GatedFusion
    - Pass P_flat to model.han.panels_to_page for page-level embeddings
    """
    with torch.no_grad():
        # Move to device
        for key in batch:
        ...
        # Create dummy images (zeros = blank/black images)
        images = torch.zeros_like(batch['images'].flatten(0, 1))
        
        # Create dummy text inputs (neutral/empty)
        input_ids = torch.zeros_like(batch['input_ids'].flatten(0, 1))
        attention_mask = torch.zeros_like(batch['attention_mask'].flatten(0, 1))
        
        # Use actual compositional features
        comp_feats = batch['comp_feats'].flatten(0, 1)
        
        # COMP ONLY: Get compositional embeddings directly, bypassing GatedFusion
        P_flat = model.atom.comp(comp_feats)
        P = P_flat.view(B, N, -1)
        
        # 2. Page-level Understanding (using comp-only panel embeddings)
        E_page, attention_weights = model.han.panels_to_page(P, batch['panel_mask'])
        
        # 3. Reading Order Prediction (from comp-only embeddings)
