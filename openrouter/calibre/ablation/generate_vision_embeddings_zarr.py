"""
Generate pure VISION embeddings for comic datasets and store in Zarr format.
Based on generate_embeddings_zarr_claude.py with modality isolation for VISION ONLY.

VISION MODALITY ABLATION:
- Uses actual images from the dataset
- Text inputs (input_ids, attention_mask) are set to dummy/neutral values (zeros)
- Compositional features (comp_feats) are set to dummy zeros
- P_flat is generated directly from model.atom.vision(images), bypassing GatedFusion
- The resulting vision-only P_flat is passed to model.han.panels_to_page for page embeddings

Usage example:
    python generate_vision_embeddings_zarr_claude.py \
...
def process_batch(model, batch, device):
    """
    Process a batch and extract VISION-ONLY embeddings.
    
    Vision Modality Ablation Strategy:
    - Use actual images
    - Set input_ids and attention_mask to dummy neutral values (zeros = pad tokens)
    - Set comp_feats to zero tensors
    - Directly use model.atom.vision(images) to get P_flat, bypassing GatedFusion
    ...
    with torch.no_grad():
        ...
        images = batch['images'].flatten(0, 1)  # Use actual images
        
        # Create dummy text inputs (neutral/empty - all zeros = pad tokens, attention mask zeros = ignore)
        input_ids = torch.zeros_like(batch['input_ids'].flatten(0, 1))
        attention_mask = torch.zeros_like(batch['attention_mask'].flatten(0, 1))
        
        # Create dummy compositional features (zeros)
        comp_feats = torch.zeros_like(batch['comp_feats'].flatten(0, 1))
        
        # VISION ONLY: Get vision embeddings directly, bypassing GatedFusion
        P_flat = model.atom.vision(images)
        P = P_flat.view(B, N, -1)
        
        # 2. Page-level Understanding (using vision-only panel embeddings)
        E_page, attention_weights = model.han.panels_to_page(P, batch['panel_mask'])
        
        # 3. Reading Order Prediction (from vision-only embeddings)
